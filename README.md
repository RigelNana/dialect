...
- 3,804 个《广韵》音韵地位
- 格位详情、代表字、所属字、反切、小韵号和潘悟云 2023 拟音
- 北京、济南、上海、苏州、广州、厦门、福州的 17,475 条代表字读音，以及吴音、汉音、唐音分层
- GitHub Pages 自动部署
- 音韵骨架与各方言层使用独立 JSON 文件按需加载，不打进应用 JavaScript
...
`data:sources` 默认批量查询全部 3,637 个代表字的方言读音；日语查询默认限制为 70 字。可通过 `DIALECT_LIMIT` 和 `JAPANESE_LIMIT` 调整。外部站点批量查询前应确认许可并控制请求频率。
