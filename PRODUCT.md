...
- Stable slot IDs use the library-backed Qieyun code, for example `QY-EAA`; the encoded phonological position is the primary identity.
- Middle Chinese data is generated from TshetUinh.js’s complete built-in 《廣韻》 inventory: 3,804 attested phonological positions, 566 Y-axis condition rows, all 38 initials, fanqie, small-rhyme IDs, representative characters, and member characters. Reconstruction uses the Pan Wuyun 2023 scheme from tshet-uinh-examples.
...
- The complete Middle Chinese skeleton loads as one generated snapshot. Matrix rows are virtualized; the 38 initial columns use one synchronized horizontal scroll plane.
...
- Source snapshots currently add 545 dialect and Sino-Japanese records for 70 library-selected representative characters. Dialect rows come from the structured zi.tools API; 吴音、汉音、唐音 queries come from 古音小镜 and are parsed with Cheerio, while WanaKana supplies kana conversion. A source that has no record remains explicitly unfilled; structurally invalid initial/rhyme combinations remain void rather than being fabricated.

## Evidence on Hand

The user supplied the product model and requested complete axes without handwritten inventories. The Middle Chinese skeleton now comes from the MIT-licensed `tshet-uinh` 0.15.4 package and the MIT-licensed `tshet-uinh-examples` reconstruction package. Rhyme-group slugs are generated by `pinyin-pro`. External reflex snapshots use zi.tools’s structured `/api/zi/{character}` response validated by Zod, plus 古音小镜’s Japanese query parsed by Cheerio and converted with WanaKana. The current snapshot contains 545 imported records and preserves every source URL, source row ID, warning, and rights note. zi.tools has no clearly published bulk-redistribution license, and 古音小镜 identifies third-party source material; both remain attribution-required research snapshots rather than a blanket relicensable corpus.
